...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [GnssObservationSetFull].
         *
    ...
    /** A builder for [GnssObservationSetFull]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(gnssObservationSetFull: GnssObservationSetFull) = apply {
            classificationMarking = gnssObservationSetFull.classificationMarking
            dataMode = gnssObservationSetFull.dataMode
            source = gnssObservationSetFull.source
            ts = gnssObservationSetFull.ts
            id = gnssObservationSetFull.id
            agcState = gnssObservationSetFull.agcState
            alt = gnssObservationSetFull.alt
            boresight = gnssObservationSetFull.boresight.map { it.toMutableList() }
            createdAt = gnssObservationSetFull.createdAt
            createdBy = gnssObservationSetFull.createdBy
            esId = gnssObservationSetFull.esId
            eventId = gnssObservationSetFull.eventId
            gDop = gnssObservationSetFull.gDop
            gnssObservationList =
                gnssObservationSetFull.gnssObservationList.map { it.toMutableList() }
            hDop = gnssObservationSetFull.hDop
            idOnOrbit = gnssObservationSetFull.idOnOrbit
            lat = gnssObservationSetFull.lat
            lon = gnssObservationSetFull.lon
            markerType = gnssObservationSetFull.markerType
            navigationStatus = gnssObservationSetFull.navigationStatus
            obsCodes = gnssObservationSetFull.obsCodes.map { it.toMutableList() }
            origin = gnssObservationSetFull.origin
            origNetwork = gnssObservationSetFull.origNetwork
            origObjectId = gnssObservationSetFull.origObjectId
            outage = gnssObservationSetFull.outage
            pDop = gnssObservationSetFull.pDop
            quat = gnssObservationSetFull.quat.map { it.toMutableList() }
            receiver = gnssObservationSetFull.receiver
            satNo = gnssObservationSetFull.satNo
            satPosition = gnssObservationSetFull.satPosition.map { it.toMutableList() }
            satVelocity = gnssObservationSetFull.satVelocity.map { it.toMutableList() }
            srcIds = gnssObservationSetFull.srcIds.map { it.toMutableList() }
            srcTyps = gnssObservationSetFull.srcTyps.map { it.toMutableList() }
            tags = gnssObservationSetFull.tags.map { it.toMutableList() }
            tDop = gnssObservationSetFull.tDop
            trackingStatus = gnssObservationSetFull.trackingStatus
            transactionId = gnssObservationSetFull.transactionId
            vDop = gnssObservationSetFull.vDop
            additionalProperties = gnssObservationSetFull.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [GnssObservationSetFull].
         *
         ...
         */
        fun build(): GnssObservationSetFull =
    ...
    fun validate(): GnssObservationSetFull = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun toString() =
        "GnssObservationSetFull{classificationMarking=$classificationMarking, dataMode=$dataMode, source=$source, ts=$ts, id=$id, agcState=$agcState, alt=$alt, boresight=$boresight, createdAt=$createdAt, createdBy=$createdBy, esId=$esId, eventId=$eventId, gDop=$gDop, gnssObservationList=$gnssObservationList, hDop=$hDop, idOnOrbit=$idOnOrbit, lat=$lat, lon=$lon, markerType=$markerType, navigationStatus=$navigationStatus, obsCodes=$obsCodes, origin=$origin, origNetwork=$origNetwork, origObjectId=$origObjectId, outage=$outage, pDop=$pDop, quat=$quat, receiver=$receiver, satNo=$satNo, satPosition=$satPosition, satVelocity=$satVelocity, srcIds=$srcIds, srcTyps=$srcTyps, tags=$tags, tDop=$tDop, trackingStatus=$trackingStatus, transactionId=$transactionId, vDop=$vDop, additionalProperties=$additionalProperties}"
}
