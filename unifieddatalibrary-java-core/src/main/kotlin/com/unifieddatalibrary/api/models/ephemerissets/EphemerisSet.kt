...
class EphemerisSet private constructor(
    ...
    private val ephemerisList: JsonField<List<EphemerisFull>>,
    ...
    private val stateVector: JsonField<StateVectorFull>,
    ...
    private constructor(
        ...
        @JsonProperty("ephemerisList") @ExcludeMissing ephemerisList: JsonField<List<EphemerisFull>> = JsonMissing.of(),
        ...
        @JsonProperty("stateVector") @ExcludeMissing stateVector: JsonField<StateVectorFull> = JsonMissing.of(),
     ...
     * @throws UnifieddatalibraryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun ephemerisList(): Optional<List<EphemerisFull>> = ephemerisList.getOptional("ephemerisList")
     ...
     * @throws UnifieddatalibraryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun stateVector(): Optional<StateVectorFull> = stateVector.getOptional("stateVector")
    ...
    @ExcludeMissing
    fun _ephemerisList(): JsonField<List<EphemerisFull>> = ephemerisList
    ...
    @ExcludeMissing
    fun _stateVector(): JsonField<StateVectorFull> = stateVector
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [EphemerisSet].
         *
    ...
    class Builder internal constructor() {
        ...
        private var ephemerisList: JsonField<MutableList<EphemerisFull>>? = null
        ...
        private var stateVector: JsonField<StateVectorFull> = JsonMissing.of()
        ...
        @JvmSynthetic
        internal fun from(ephemerisSet: EphemerisSet) =
            apply {
                category = ephemerisSet.category
                classificationMarking = ephemerisSet.classificationMarking
                dataMode = ephemerisSet.dataMode
                numPoints = ephemerisSet.numPoints
                pointEndTime = ephemerisSet.pointEndTime
                pointStartTime = ephemerisSet.pointStartTime
                source = ephemerisSet.source
                type = ephemerisSet.type
                id = ephemerisSet.id
                bDot = ephemerisSet.bDot
                centBody = ephemerisSet.centBody
                comments = ephemerisSet.comments
                covReferenceFrame = ephemerisSet.covReferenceFrame
                createdAt = ephemerisSet.createdAt
                createdBy = ephemerisSet.createdBy
                description = ephemerisSet.description
                descriptor = ephemerisSet.descriptor
                dragModel = ephemerisSet.dragModel
                edr = ephemerisSet.edr
                ephemerisList = ephemerisSet.ephemerisList.map { it.toMutableList() }
                filename = ephemerisSet.filename
                geopotentialModel = ephemerisSet.geopotentialModel
                hasAccel = ephemerisSet.hasAccel
                hasCov = ephemerisSet.hasCov
                hasMnvr = ephemerisSet.hasMnvr
                idManeuvers = ephemerisSet.idManeuvers.map { it.toMutableList() }
                idOnOrbit = ephemerisSet.idOnOrbit
                idStateVector = ephemerisSet.idStateVector
                integrator = ephemerisSet.integrator
                interpolation = ephemerisSet.interpolation
                interpolationDegree = ephemerisSet.interpolationDegree
                lunarSolar = ephemerisSet.lunarSolar
                onOrbit = ephemerisSet.onOrbit
                origin = ephemerisSet.origin
                origNetwork = ephemerisSet.origNetwork
                origObjectId = ephemerisSet.origObjectId
                pedigree = ephemerisSet.pedigree
                referenceFrame = ephemerisSet.referenceFrame
                satNo = ephemerisSet.satNo
                solidEarthTides = ephemerisSet.solidEarthTides
                stateVector = ephemerisSet.stateVector
                stepSize = ephemerisSet.stepSize
                tags = ephemerisSet.tags.map { it.toMutableList() }
                transactionId = ephemerisSet.transactionId
                usableEndTime = ephemerisSet.usableEndTime
                usableStartTime = ephemerisSet.usableStartTime
                additionalProperties = ephemerisSet.additionalProperties.toMutableMap()
            }
        ...
        /** The list of ephemeris states belonging to the EphemerisSet. Each ephemeris point is associated with a parent Ephemeris Set via the EphemerisSet ID (esId). */
        fun ephemerisList(ephemerisList: List<EphemerisFull>) = ephemerisList(JsonField.of(ephemerisList))

        /**
         * Sets [Builder.ephemerisList] to an arbitrary JSON value.
         *
         * You should usually call [Builder.ephemerisList] with a well-typed `List<EphemerisFull>` value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun ephemerisList(ephemerisList: JsonField<List<EphemerisFull>>) =
            apply {
                this.ephemerisList = ephemerisList.map { it.toMutableList() }
            }

        /**
         * Adds a single [EphemerisFull] to [Builder.ephemerisList].
         *
         * @throws IllegalStateException if the field was previously set to a non-list.
         */
        fun addEphemerisList(ephemerisList: EphemerisFull) =
            apply {
        ...
        /** This service provides operations for querying and manipulation of state vectors for OnOrbit objects. State vectors are cartesian vectors of position (r) and velocity (v) that, together with their time (epoch) (t), uniquely determine the trajectory of the orbiting body in space. J2000 is the preferred coordinate frame for all state vector positions/velocities in UDL, but in some cases data may be in another frame depending on the provider and/or datatype. Please see the 'Discover' tab in the storefront to confirm coordinate frames by data provider. */
        fun stateVector(stateVector: StateVectorFull) = stateVector(JsonField.of(stateVector))

        /**
         * Sets [Builder.stateVector] to an arbitrary JSON value.
         *
         * You should usually call [Builder.stateVector] with a well-typed [StateVectorFull] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun stateVector(stateVector: JsonField<StateVectorFull>) =
            apply {
        ...
        /**
         * Returns an immutable instance of [EphemerisSet].
         *
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (category.asKnown().isPresent) 1 else 0) + (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (numPoints.asKnown().isPresent) 1 else 0) + (if (pointEndTime.asKnown().isPresent) 1 else 0) + (if (pointStartTime.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (type.asKnown().isPresent) 1 else 0) + (if (id.asKnown().isPresent) 1 else 0) + (if (bDot.asKnown().isPresent) 1 else 0) + (if (centBody.asKnown().isPresent) 1 else 0) + (if (comments.asKnown().isPresent) 1 else 0) + (covReferenceFrame.asKnown().getOrNull()?.validity() ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (descriptor.asKnown().isPresent) 1 else 0) + (if (dragModel.asKnown().isPresent) 1 else 0) + (if (edr.asKnown().isPresent) 1 else 0) + (ephemerisList.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (filename.asKnown().isPresent) 1 else 0) + (if (geopotentialModel.asKnown().isPresent) 1 else 0) + (if (hasAccel.asKnown().isPresent) 1 else 0) + (if (hasCov.asKnown().isPresent) 1 else 0) + (if (hasMnvr.asKnown().isPresent) 1 else 0) + (idManeuvers.asKnown().getOrNull()?.size ?: 0) + (if (idOnOrbit.asKnown().isPresent) 1 else 0) + (if (idStateVector.asKnown().isPresent) 1 else 0) + (if (integrator.asKnown().isPresent) 1 else 0) + (if (interpolation.asKnown().isPresent) 1 else 0) + (if (interpolationDegree.asKnown().isPresent) 1 else 0) + (if (lunarSolar.asKnown().isPresent) 1 else 0) + (onOrbit.asKnown().getOrNull()?.validity() ?: 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (origObjectId.asKnown().isPresent) 1 else 0) + (if (pedigree.asKnown().isPresent) 1 else 0) + (referenceFrame.asKnown().getOrNull()?.validity() ?: 0) + (if (satNo.asKnown().isPresent) 1 else 0) + (if (solidEarthTides.asKnown().isPresent) 1 else 0) + (stateVector.asKnown().getOrNull()?.validity() ?: 0) + (if (stepSize.asKnown().isPresent) 1 else 0) + (tags.asKnown().getOrNull()?.size ?: 0) + (if (transactionId.asKnown().isPresent) 1 else 0) + (if (usableEndTime.asKnown().isPresent) 1 else 0) + (if (usableStartTime.asKnown().isPresent) 1 else 0)
