...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [SgiGetResponse].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(sgiGetResponse: SgiGetResponse) = apply {
            classificationMarking = sgiGetResponse.classificationMarking
            dataMode = sgiGetResponse.dataMode
            effectiveDate = sgiGetResponse.effectiveDate
            sgiDate = sgiGetResponse.sgiDate
            source = sgiGetResponse.source
            id = sgiGetResponse.id
            analyzerAttenuation = sgiGetResponse.analyzerAttenuation
            ap = sgiGetResponse.ap
            apDuration = sgiGetResponse.apDuration
            coeffDegree = sgiGetResponse.coeffDegree.map { it.toMutableList() }
            coeffOrder = sgiGetResponse.coeffOrder.map { it.toMutableList() }
            createdAt = sgiGetResponse.createdAt
            createdBy = sgiGetResponse.createdBy
            ctce = sgiGetResponse.ctce.map { it.toMutableList() }
            ctci = sgiGetResponse.ctci.map { it.toMutableList() }
            dst = sgiGetResponse.dst
            dtc = sgiGetResponse.dtc
            e10 = sgiGetResponse.e10
            e54 = sgiGetResponse.e54
            f10 = sgiGetResponse.f10
            f10High = sgiGetResponse.f10High
            f10Low = sgiGetResponse.f10Low
            f54 = sgiGetResponse.f54
            f81 = sgiGetResponse.f81
            frequencies = sgiGetResponse.frequencies.map { it.toMutableList() }
            gamma = sgiGetResponse.gamma
            idSensor = sgiGetResponse.idSensor
            kIndex = sgiGetResponse.kIndex
            kp = sgiGetResponse.kp
            kpDuration = sgiGetResponse.kpDuration
            m10 = sgiGetResponse.m10
            m54 = sgiGetResponse.m54
            mode = sgiGetResponse.mode
            normFactor = sgiGetResponse.normFactor
            observedBaseline = sgiGetResponse.observedBaseline.map { it.toMutableList() }
            origin = sgiGetResponse.origin
            origNetwork = sgiGetResponse.origNetwork
            origSensorId = sgiGetResponse.origSensorId
            powers = sgiGetResponse.powers.map { it.toMutableList() }
            precedence = sgiGetResponse.precedence
            rawFileUri = sgiGetResponse.rawFileUri
            rbDuration = sgiGetResponse.rbDuration
            rbIndex = sgiGetResponse.rbIndex
            rbRegionCode = sgiGetResponse.rbRegionCode
            s10 = sgiGetResponse.s10
            s54 = sgiGetResponse.s54
            state = sgiGetResponse.state
            stationName = sgiGetResponse.stationName
            stce = sgiGetResponse.stce.map { it.toMutableList() }
            stci = sgiGetResponse.stci.map { it.toMutableList() }
            sunspotNum = sgiGetResponse.sunspotNum
            sunspotNumHigh = sgiGetResponse.sunspotNumHigh
            sunspotNumLow = sgiGetResponse.sunspotNumLow
            tags = sgiGetResponse.tags.map { it.toMutableList() }
            transactionId = sgiGetResponse.transactionId
            type = sgiGetResponse.type
            updatedAt = sgiGetResponse.updatedAt
            updatedBy = sgiGetResponse.updatedBy
            y10 = sgiGetResponse.y10
            y54 = sgiGetResponse.y54
            additionalProperties = sgiGetResponse.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [SgiGetResponse].
         *
    ...
    fun validate(): SgiGetResponse = apply {
        if (validated) {
            return@apply
        }
