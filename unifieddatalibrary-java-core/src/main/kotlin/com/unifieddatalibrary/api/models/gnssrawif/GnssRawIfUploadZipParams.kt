 ...
 * file extension whose content conforms to the `GNSSRawIF_Ingest` schema.\ 2\) A file with the
 * `.hdf5` file extension.
 *
 * The JSON and HDF5 files will be associated with each other via the `id` field. Query the metadata
 * via `GET /udl/gnssrawif` and use `GET /udl/gnssrawif/getFile/{id}` to retrieve the HDF5 file.
 *
...
private constructor(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is GnssRawIfUploadZipParams &&
            body == other.body &&
