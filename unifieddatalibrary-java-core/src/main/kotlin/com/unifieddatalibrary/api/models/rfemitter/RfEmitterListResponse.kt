...
class RfEmitterListResponse private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(rfEmitterListResponse: RfEmitterListResponse) =
            apply {
    ...
    fun validate(): RfEmitterListResponse =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (idEntity.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (type.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return /* spotless:off */ other is RfEmitterListResponse && classificationMarking == other.classificationMarking && dataMode == other.dataMode && name == other.name && source == other.source && id == other.id && createdAt == other.createdAt && createdBy == other.createdBy && idEntity == other.idEntity && origin == other.origin && origNetwork == other.origNetwork && type == other.type && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(classificationMarking, dataMode, name, source, id, createdAt, createdBy, idEntity, origin, origNetwork, type, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() = "RfEmitterListResponse{classificationMarking=$classificationMarking, dataMode=$dataMode, name=$name, source=$source, id=$id, createdAt=$createdAt, createdBy=$createdBy, idEntity=$idEntity, origin=$origin, origNetwork=$origNetwork, type=$type, additionalProperties=$additionalProperties}"
}
