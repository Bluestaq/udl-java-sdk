...
class OnorbitIngest private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [OnorbitIngest].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(onorbitIngest: OnorbitIngest) =
            apply {
                classificationMarking = onorbitIngest.classificationMarking
                dataMode = onorbitIngest.dataMode
                satNo = onorbitIngest.satNo
                source = onorbitIngest.source
                altName = onorbitIngest.altName
                category = onorbitIngest.category
                commonName = onorbitIngest.commonName
                constellation = onorbitIngest.constellation
                countryCode = onorbitIngest.countryCode
                createdAt = onorbitIngest.createdAt
                createdBy = onorbitIngest.createdBy
                decayDate = onorbitIngest.decayDate
                idOnOrbit = onorbitIngest.idOnOrbit
                intlDes = onorbitIngest.intlDes
                launchDate = onorbitIngest.launchDate
                launchSiteId = onorbitIngest.launchSiteId
                lifetimeYears = onorbitIngest.lifetimeYears
                missionNumber = onorbitIngest.missionNumber
                objectType = onorbitIngest.objectType
                origin = onorbitIngest.origin
                origNetwork = onorbitIngest.origNetwork
                additionalProperties = onorbitIngest.additionalProperties.toMutableMap()
            }
        ...
        /**
         * Returns an immutable instance of [OnorbitIngest].
         *
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (satNo.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (altName.asKnown().isPresent) 1 else 0) + (category.asKnown().getOrNull()?.validity() ?: 0) + (if (commonName.asKnown().isPresent) 1 else 0) + (if (constellation.asKnown().isPresent) 1 else 0) + (if (countryCode.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (decayDate.asKnown().isPresent) 1 else 0) + (if (idOnOrbit.asKnown().isPresent) 1 else 0) + (if (intlDes.asKnown().isPresent) 1 else 0) + (if (launchDate.asKnown().isPresent) 1 else 0) + (if (launchSiteId.asKnown().isPresent) 1 else 0) + (if (lifetimeYears.asKnown().isPresent) 1 else 0) + (if (missionNumber.asKnown().isPresent) 1 else 0) + (objectType.asKnown().getOrNull()?.validity() ?: 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0)
    ...
    override fun toString() = "OnorbitIngest{classificationMarking=$classificationMarking, dataMode=$dataMode, satNo=$satNo, source=$source, altName=$altName, category=$category, commonName=$commonName, constellation=$constellation, countryCode=$countryCode, createdAt=$createdAt, createdBy=$createdBy, decayDate=$decayDate, idOnOrbit=$idOnOrbit, intlDes=$intlDes, launchDate=$launchDate, launchSiteId=$launchSiteId, lifetimeYears=$lifetimeYears, missionNumber=$missionNumber, objectType=$objectType, origin=$origin, origNetwork=$origNetwork, additionalProperties=$additionalProperties}"
}
