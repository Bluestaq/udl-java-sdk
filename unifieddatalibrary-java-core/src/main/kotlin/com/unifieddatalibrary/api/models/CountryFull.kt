...
class CountryFull private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(countryFull: CountryFull) =
            apply {
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (code.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (codeAlt.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (fipsCode.asKnown().isPresent) 1 else 0) + (if (iso3Code.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (updatedBy.asKnown().isPresent) 1 else 0)
    ...
    override fun toString() = "CountryFull{code=$code, dataMode=$dataMode, source=$source, codeAlt=$codeAlt, createdAt=$createdAt, createdBy=$createdBy, fipsCode=$fipsCode, iso3Code=$iso3Code, name=$name, origNetwork=$origNetwork, updatedAt=$updatedAt, updatedBy=$updatedBy, additionalProperties=$additionalProperties}"
}
