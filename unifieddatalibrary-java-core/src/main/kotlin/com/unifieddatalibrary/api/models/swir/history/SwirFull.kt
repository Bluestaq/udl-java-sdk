...
private constructor(
     ...
     */
    @JsonProperty("absFluxes") @ExcludeMissing fun _absFluxes(): JsonField<List<Double>> = absFluxes
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [SwirFull].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(swirFull: SwirFull) = apply {
            classificationMarking = swirFull.classificationMarking
            dataMode = swirFull.dataMode
            source = swirFull.source
            ts = swirFull.ts
            id = swirFull.id
            absFluxes = swirFull.absFluxes.map { it.toMutableList() }
            badWave = swirFull.badWave
            createdAt = swirFull.createdAt
            createdBy = swirFull.createdBy
            fluxRatios = swirFull.fluxRatios.map { it.toMutableList() }
            idOnOrbit = swirFull.idOnOrbit
            lat = swirFull.lat
            locationName = swirFull.locationName
            lon = swirFull.lon
            onOrbit = swirFull.onOrbit
            origin = swirFull.origin
            origNetwork = swirFull.origNetwork
            origObjectId = swirFull.origObjectId
            ratioWavelengths = swirFull.ratioWavelengths.map { it.toMutableList() }
            satNo = swirFull.satNo
            solarPhaseAngle = swirFull.solarPhaseAngle
            wavelengths = swirFull.wavelengths.map { it.toMutableList() }
            additionalProperties = swirFull.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [SwirFull].
         *
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is SwirFull && classificationMarking == other.classificationMarking && dataMode == other.dataMode && source == other.source && ts == other.ts && id == other.id && absFluxes == other.absFluxes && badWave == other.badWave && createdAt == other.createdAt && createdBy == other.createdBy && fluxRatios == other.fluxRatios && idOnOrbit == other.idOnOrbit && lat == other.lat && locationName == other.locationName && lon == other.lon && onOrbit == other.onOrbit && origin == other.origin && origNetwork == other.origNetwork && origObjectId == other.origObjectId && ratioWavelengths == other.ratioWavelengths && satNo == other.satNo && solarPhaseAngle == other.solarPhaseAngle && wavelengths == other.wavelengths && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(classificationMarking, dataMode, source, ts, id, absFluxes, badWave, createdAt, createdBy, fluxRatios, idOnOrbit, lat, locationName, lon, onOrbit, origin, origNetwork, origObjectId, ratioWavelengths, satNo, solarPhaseAngle, wavelengths, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "SwirFull{classificationMarking=$classificationMarking, dataMode=$dataMode, source=$source, ts=$ts, id=$id, absFluxes=$absFluxes, badWave=$badWave, createdAt=$createdAt, createdBy=$createdBy, fluxRatios=$fluxRatios, idOnOrbit=$idOnOrbit, lat=$lat, locationName=$locationName, lon=$lon, onOrbit=$onOrbit, origin=$origin, origNetwork=$origNetwork, origObjectId=$origObjectId, ratioWavelengths=$ratioWavelengths, satNo=$satNo, solarPhaseAngle=$solarPhaseAngle, wavelengths=$wavelengths, additionalProperties=$additionalProperties}"
}
