...
class SensorPlanUpdateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(sensorPlanUpdateParams: SensorPlanUpdateParams) =
            apply {
        ...
        /** The type of records that comprise this parent set (COLLECT, CONTACT). */
        fun recType(recType: String) =
            apply {
                body.recType(recType)
            }
        ...
        /** The sensor or ground network associated with this plan or schedule. */
        fun senNetwork(senNetwork: String) =
            apply {
                body.senNetwork(senNetwork)
            }
    ...
    class Body private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (recType.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (startTime.asKnown().isPresent) 1 else 0) + (if (type.asKnown().isPresent) 1 else 0) + (if (bodyId.asKnown().isPresent) 1 else 0) + (collectRequests.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (customer.asKnown().isPresent) 1 else 0) + (if (endTime.asKnown().isPresent) 1 else 0) + (if (idSensor.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (origSensorId.asKnown().isPresent) 1 else 0) + (if (purpose.asKnown().isPresent) 1 else 0) + (if (reqTotal.asKnown().isPresent) 1 else 0) + (if (senNetwork.asKnown().isPresent) 1 else 0) + (if (status.asKnown().isPresent) 1 else 0) + (tags.asKnown().getOrNull()?.size ?: 0)
        ...
        override fun toString() = "Body{classificationMarking=$classificationMarking, dataMode=$dataMode, recType=$recType, source=$source, startTime=$startTime, type=$type, bodyId=$bodyId, collectRequests=$collectRequests, createdAt=$createdAt, createdBy=$createdBy, customer=$customer, endTime=$endTime, idSensor=$idSensor, name=$name, origin=$origin, origNetwork=$origNetwork, origSensorId=$origSensorId, purpose=$purpose, reqTotal=$reqTotal, senNetwork=$senNetwork, status=$status, tags=$tags, additionalProperties=$additionalProperties}"
    }
    ...
    override fun toString() = "SensorPlanUpdateParams{pathId=$pathId, body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
