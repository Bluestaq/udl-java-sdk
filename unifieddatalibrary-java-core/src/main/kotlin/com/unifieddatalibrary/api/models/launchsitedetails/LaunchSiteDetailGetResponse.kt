...
class LaunchSiteDetailGetResponse private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("availableInclinations") @ExcludeMissing availableInclinations: JsonField<List<Double>> = JsonMissing.of(),
        ...
        @JsonProperty("launchGroup") @ExcludeMissing launchGroup: JsonField<String> = JsonMissing.of(),
     ...
     * @throws UnifieddatalibraryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun availableInclinations(): Optional<List<Double>> = availableInclinations.getOptional("availableInclinations")
     ...
     *
     * Unlike [availableInclinations], this method doesn't throw if the JSON field has an unexpected type.
     */
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(launchSiteDetailGetResponse: LaunchSiteDetailGetResponse) =
            apply {
                ...
                availableInclinations = launchSiteDetailGetResponse.availableInclinations.map { it.toMutableList() }
        ...
        /** Array of supported launch inclinations known for this site. The array is always sized as a multiple of two and includes start/end values to support ranges. For example, if a site support inclinations of 10 and 12-14, the array would have the following values: [10,10, 12,14]. */
        fun availableInclinations(availableInclinations: List<Double>) = availableInclinations(JsonField.of(availableInclinations))

        /**
         * Sets [Builder.availableInclinations] to an arbitrary JSON value.
         *
         * You should usually call [Builder.availableInclinations] with a well-typed `List<Double>` value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun availableInclinations(availableInclinations: JsonField<List<Double>>) =
            apply {
         ...
         */
        fun addAvailableInclination(availableInclination: Double) =
            apply {
                availableInclinations = (availableInclinations ?: JsonField.of(mutableListOf())).also {
         ...
         *
         * You should usually call [Builder.launchGroup] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun launchGroup(launchGroup: JsonField<String>) =
            apply {
    ...
    fun validate(): LaunchSiteDetailGetResponse =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (idLaunchSite.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (id.asKnown().isPresent) 1 else 0) + (availableInclinations.asKnown().getOrNull()?.size ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (idLocation.asKnown().isPresent) 1 else 0) + (if (launchGroup.asKnown().isPresent) 1 else 0) + (location.asKnown().getOrNull()?.validity() ?: 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (tags.asKnown().getOrNull()?.size ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (updatedBy.asKnown().isPresent) 1 else 0)
