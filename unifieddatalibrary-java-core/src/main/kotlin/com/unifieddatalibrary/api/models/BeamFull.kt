...
class BeamFull private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("beamContours") @ExcludeMissing beamContours: JsonField<List<BeamcontourFull>> = JsonMissing.of(),
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(beamFull: BeamFull) =
            apply {
        ...
        /** Read-only collection of beamContours, ignored on create/update. These details must be created separately via the /udl/beamcontour operations. */
        fun beamContours(beamContours: List<BeamcontourFull>) = beamContours(JsonField.of(beamContours))

        /**
         * Sets [Builder.beamContours] to an arbitrary JSON value.
         *
         * You should usually call [Builder.beamContours] with a well-typed `List<BeamcontourFull>` value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun beamContours(beamContours: JsonField<List<BeamcontourFull>>) =
            apply {
         ...
         */
        fun addBeamContour(beamContour: BeamcontourFull) =
            apply {
                beamContours = (beamContours ?: JsonField.of(mutableListOf())).also {
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (beamName.asKnown().isPresent) 1 else 0) + (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (id.asKnown().isPresent) 1 else 0) + (beamContours.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (notes.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (updatedBy.asKnown().isPresent) 1 else 0)
    ...
    override fun toString() = "BeamFull{beamName=$beamName, classificationMarking=$classificationMarking, dataMode=$dataMode, source=$source, id=$id, beamContours=$beamContours, createdAt=$createdAt, createdBy=$createdBy, notes=$notes, origin=$origin, origNetwork=$origNetwork, updatedAt=$updatedAt, updatedBy=$updatedBy, additionalProperties=$additionalProperties}"
}
