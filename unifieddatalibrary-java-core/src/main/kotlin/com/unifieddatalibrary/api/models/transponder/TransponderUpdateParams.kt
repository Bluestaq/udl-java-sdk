...
class TransponderUpdateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(transponderUpdateParams: TransponderUpdateParams) =
            apply {
    ...
    class Body private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (idComm.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (bodyId.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (fec.asKnown().isPresent) 1 else 0) + (if (format.asKnown().isPresent) 1 else 0) + (if (modulation.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (nid.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (symbolRate.asKnown().isPresent) 1 else 0) + (if (system.asKnown().isPresent) 1 else 0) + (if (tid.asKnown().isPresent) 1 else 0) + (if (ttf.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return /* spotless:off */ other is TransponderUpdateParams && pathId == other.pathId && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams /* spotless:on */
    }

    override fun hashCode(): Int = /* spotless:off */ Objects.hash(pathId, body, additionalHeaders, additionalQueryParams) /* spotless:on */

    override fun toString() = "TransponderUpdateParams{pathId=$pathId, body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
