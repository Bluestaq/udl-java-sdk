...
/**
 * An entity is a generic representation of any object within a space/SSA system such as sensors,
 * on-orbit objects, RF Emitters, space craft buses, etc. An entity can have an operating unit, a
...
private constructor(
    ...
    /**
     * Onorbit identifier if this entity is part of an on-orbit object. For the public catalog, the
     * idOnOrbit is typically the satellite number as a string, but may be a UUID for analyst or
     ...
     */
    @JsonProperty("ownerType") @ExcludeMissing fun _ownerType(): JsonField<OwnerType> = ownerType
     ...
     */
    @JsonProperty("rfBands") @ExcludeMissing fun _rfBands(): JsonField<List<RfBandFull>> = rfBands
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [EntityFull].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(entityFull: EntityFull) = apply {
            classificationMarking = entityFull.classificationMarking
            dataMode = entityFull.dataMode
            name = entityFull.name
            source = entityFull.source
            type = entityFull.type
            countryCode = entityFull.countryCode
            createdAt = entityFull.createdAt
            createdBy = entityFull.createdBy
            idEntity = entityFull.idEntity
            idLocation = entityFull.idLocation
            idOnOrbit = entityFull.idOnOrbit
            idOperatingUnit = entityFull.idOperatingUnit
            location = entityFull.location
            onOrbit = entityFull.onOrbit
            operatingUnit = entityFull.operatingUnit
            origin = entityFull.origin
            origNetwork = entityFull.origNetwork
            ownerType = entityFull.ownerType
            rfBands = entityFull.rfBands.map { it.toMutableList() }
            statusCollection = entityFull.statusCollection.map { it.toMutableList() }
            taskable = entityFull.taskable
            updatedAt = entityFull.updatedAt
            updatedBy = entityFull.updatedBy
            urls = entityFull.urls.map { it.toMutableList() }
            additionalProperties = entityFull.additionalProperties.toMutableMap()
        }
         ...
         *
         * You should usually call [Builder.statusCollection] with a well-typed `List<StatusFull>`
         * value instead. This method is primarily for setting the field to an undocumented or not
         * yet supported value.
         */
        ...
        /**
         * Returns an immutable instance of [EntityFull].
         *
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is EntityFull && classificationMarking == other.classificationMarking && dataMode == other.dataMode && name == other.name && source == other.source && type == other.type && countryCode == other.countryCode && createdAt == other.createdAt && createdBy == other.createdBy && idEntity == other.idEntity && idLocation == other.idLocation && idOnOrbit == other.idOnOrbit && idOperatingUnit == other.idOperatingUnit && location == other.location && onOrbit == other.onOrbit && operatingUnit == other.operatingUnit && origin == other.origin && origNetwork == other.origNetwork && ownerType == other.ownerType && rfBands == other.rfBands && statusCollection == other.statusCollection && taskable == other.taskable && updatedAt == other.updatedAt && updatedBy == other.updatedBy && urls == other.urls && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(classificationMarking, dataMode, name, source, type, countryCode, createdAt, createdBy, idEntity, idLocation, idOnOrbit, idOperatingUnit, location, onOrbit, operatingUnit, origin, origNetwork, ownerType, rfBands, statusCollection, taskable, updatedAt, updatedBy, urls, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "EntityFull{classificationMarking=$classificationMarking, dataMode=$dataMode, name=$name, source=$source, type=$type, countryCode=$countryCode, createdAt=$createdAt, createdBy=$createdBy, idEntity=$idEntity, idLocation=$idLocation, idOnOrbit=$idOnOrbit, idOperatingUnit=$idOperatingUnit, location=$location, onOrbit=$onOrbit, operatingUnit=$operatingUnit, origin=$origin, origNetwork=$origNetwork, ownerType=$ownerType, rfBands=$rfBands, statusCollection=$statusCollection, taskable=$taskable, updatedAt=$updatedAt, updatedBy=$updatedBy, urls=$urls, additionalProperties=$additionalProperties}"
}
