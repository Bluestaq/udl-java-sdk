...
class SortiePprCreateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(sortiePprCreateParams: SortiePprCreateParams) =
            apply {
        ...
        /** Identifier of the prior permission required (PPR) grantor. */
        fun grantor(grantor: String) =
            apply {
                body.grantor(grantor)
            }

        /**
         * Sets [Builder.grantor] to an arbitrary JSON value.
         *
         * You should usually call [Builder.grantor] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun grantor(grantor: JsonField<String>) =
            apply {
                body.grantor(grantor)
            }
        ...
        /** The username of the prior permission required (PPR) requestor. */
        fun requestor(requestor: String) =
            apply {
    ...
    class Body private constructor(
        ...
        @ExcludeMissing
        fun _grantor(): JsonField<String> = grantor
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (idSortie.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (endTime.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (grantor.asKnown().isPresent) 1 else 0) + (if (number.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (remarks.asKnown().isPresent) 1 else 0) + (if (requestor.asKnown().isPresent) 1 else 0) + (if (sourceDl.asKnown().isPresent) 1 else 0) + (if (startTime.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0)
    ...
    override fun toString() = "SortiePprCreateParams{body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
