...
private constructor(
     ...
     * covReferenceFrame.
     *
     * If the covReferenceFrame from the EphemerisSet table is null it is assumed to be J2000.
     *
     * The array values represent the lower triangular half of the position-velocity covariance
     * matrix. The size of the covariance matrix is dynamic, depending on whether the covariance for
     * position only or position & velocity.
     *
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [EphemerisAbridged].
         *
    ...
    /** A builder for [EphemerisAbridged]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(ephemerisAbridged: EphemerisAbridged) = apply {
            classificationMarking = ephemerisAbridged.classificationMarking
            dataMode = ephemerisAbridged.dataMode
            source = ephemerisAbridged.source
            ts = ephemerisAbridged.ts
            xpos = ephemerisAbridged.xpos
            xvel = ephemerisAbridged.xvel
            ypos = ephemerisAbridged.ypos
            yvel = ephemerisAbridged.yvel
            zpos = ephemerisAbridged.zpos
            zvel = ephemerisAbridged.zvel
            id = ephemerisAbridged.id
            cov = ephemerisAbridged.cov.map { it.toMutableList() }
            createdAt = ephemerisAbridged.createdAt
            createdBy = ephemerisAbridged.createdBy
            esId = ephemerisAbridged.esId
            idOnOrbit = ephemerisAbridged.idOnOrbit
            origin = ephemerisAbridged.origin
            origObjectId = ephemerisAbridged.origObjectId
            xaccel = ephemerisAbridged.xaccel
            yaccel = ephemerisAbridged.yaccel
            zaccel = ephemerisAbridged.zaccel
            additionalProperties = ephemerisAbridged.additionalProperties.toMutableMap()
        }
         ...
         * for position only or position & velocity.
         *
         * The covariance elements are position dependent within the array with values ordered as
         * follows:
         * ```
        ...
        /**
         * Returns an immutable instance of [EphemerisAbridged].
         *
     ...
     *   its expected type.
     */
    fun validate(): EphemerisAbridged = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is EphemerisAbridged &&
    ...
    override fun toString() =
        "EphemerisAbridged{classificationMarking=$classificationMarking, dataMode=$dataMode, source=$source, ts=$ts, xpos=$xpos, xvel=$xvel, ypos=$ypos, yvel=$yvel, zpos=$zpos, zvel=$zvel, id=$id, cov=$cov, createdAt=$createdAt, createdBy=$createdBy, esId=$esId, idOnOrbit=$idOnOrbit, origin=$origin, origObjectId=$origObjectId, xaccel=$xaccel, yaccel=$yaccel, zaccel=$zaccel, additionalProperties=$additionalProperties}"
}
