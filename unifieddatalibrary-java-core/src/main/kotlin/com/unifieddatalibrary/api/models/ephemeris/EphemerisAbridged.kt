...
class EphemerisAbridged private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(ephemerisAbridged: EphemerisAbridged) =
            apply {
    ...
    fun validate(): EphemerisAbridged =
        apply {
            if (validated) {
              return@apply
            }
    ...
    override fun toString() = "EphemerisAbridged{classificationMarking=$classificationMarking, dataMode=$dataMode, source=$source, ts=$ts, xpos=$xpos, xvel=$xvel, ypos=$ypos, yvel=$yvel, zpos=$zpos, zvel=$zvel, id=$id, cov=$cov, createdAt=$createdAt, createdBy=$createdBy, esId=$esId, idOnOrbit=$idOnOrbit, origin=$origin, origObjectId=$origObjectId, xaccel=$xaccel, yaccel=$yaccel, zaccel=$zaccel, additionalProperties=$additionalProperties}"
}
