...
class HistoryQueryResponse private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(historyQueryResponse: HistoryQueryResponse) =
            apply {
                centerFreq = historyQueryResponse.centerFreq.map { it.toMutableList() }
                ...
                bitDepth = historyQueryResponse.bitDepth
                boresight = historyQueryResponse.boresight.map { it.toMutableList() }
                createdAt = historyQueryResponse.createdAt
                createdBy = historyQueryResponse.createdBy
                dataRate = historyQueryResponse.dataRate
                diffCodeBias = historyQueryResponse.diffCodeBias.map { it.toMutableList() }
                endAlt = historyQueryResponse.endAlt
                endLat = historyQueryResponse.endLat
                endLon = historyQueryResponse.endLon
                esId = historyQueryResponse.esId
                eventId = historyQueryResponse.eventId
                fileSize = historyQueryResponse.fileSize
                idOnOrbit = historyQueryResponse.idOnOrbit
                ifFreq = historyQueryResponse.ifFreq.map { it.toMutableList() }
                origin = historyQueryResponse.origin
                origNetwork = historyQueryResponse.origNetwork
                origObjectId = historyQueryResponse.origObjectId
                postFourier = historyQueryResponse.postFourier.map { it.toMutableList() }
                quat = historyQueryResponse.quat.map { it.toMutableList() }
                receiver = historyQueryResponse.receiver
                sampleRate = historyQueryResponse.sampleRate.map { it.toMutableList() }
                sampleType = historyQueryResponse.sampleType
                satNo = historyQueryResponse.satNo
                sequenceId = historyQueryResponse.sequenceId
                setId = historyQueryResponse.setId
                setLength = historyQueryResponse.setLength
                srcIds = historyQueryResponse.srcIds.map { it.toMutableList() }
                srcTyps = historyQueryResponse.srcTyps.map { it.toMutableList() }
                startAlt = historyQueryResponse.startAlt
                startIndex = historyQueryResponse.startIndex
                startLat = historyQueryResponse.startLat
                startLon = historyQueryResponse.startLon
