...
class NotificationFull private constructor(
    ...
    @ExcludeMissing
    fun _msgBody(): JsonField<String> = msgBody
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(notificationFull: NotificationFull) =
            apply {
         ...
         *
         * You should usually call [Builder.msgBody] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun msgBody(msgBody: JsonField<String>) =
            apply {
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (msgBody.asKnown().isPresent) 1 else 0) + (if (msgType.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (sourceDl.asKnown().isPresent) 1 else 0) + (tags.asKnown().getOrNull()?.size ?: 0)
    ...
    override fun toString() = "NotificationFull{classificationMarking=$classificationMarking, dataMode=$dataMode, msgBody=$msgBody, msgType=$msgType, source=$source, id=$id, createdAt=$createdAt, createdBy=$createdBy, origin=$origin, origNetwork=$origNetwork, sourceDl=$sourceDl, tags=$tags, additionalProperties=$additionalProperties}"
}
