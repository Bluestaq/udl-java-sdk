...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [SgiListResponse].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(sgiListResponse: SgiListResponse) = apply {
            classificationMarking = sgiListResponse.classificationMarking
            dataMode = sgiListResponse.dataMode
            effectiveDate = sgiListResponse.effectiveDate
            sgiDate = sgiListResponse.sgiDate
            source = sgiListResponse.source
            id = sgiListResponse.id
            analyzerAttenuation = sgiListResponse.analyzerAttenuation
            ap = sgiListResponse.ap
            apDuration = sgiListResponse.apDuration
            coeffDegree = sgiListResponse.coeffDegree.map { it.toMutableList() }
            coeffOrder = sgiListResponse.coeffOrder.map { it.toMutableList() }
            createdAt = sgiListResponse.createdAt
            createdBy = sgiListResponse.createdBy
            ctce = sgiListResponse.ctce.map { it.toMutableList() }
            ctci = sgiListResponse.ctci.map { it.toMutableList() }
            dst = sgiListResponse.dst
            dtc = sgiListResponse.dtc
            e10 = sgiListResponse.e10
            e54 = sgiListResponse.e54
            f10 = sgiListResponse.f10
            f10High = sgiListResponse.f10High
            f10Low = sgiListResponse.f10Low
            f54 = sgiListResponse.f54
            f81 = sgiListResponse.f81
            gamma = sgiListResponse.gamma
            idSensor = sgiListResponse.idSensor
            kIndex = sgiListResponse.kIndex
            kp = sgiListResponse.kp
            kpDuration = sgiListResponse.kpDuration
            m10 = sgiListResponse.m10
            m54 = sgiListResponse.m54
            mode = sgiListResponse.mode
            normFactor = sgiListResponse.normFactor
            origin = sgiListResponse.origin
            origNetwork = sgiListResponse.origNetwork
            origSensorId = sgiListResponse.origSensorId
            precedence = sgiListResponse.precedence
            rawFileUri = sgiListResponse.rawFileUri
            rbDuration = sgiListResponse.rbDuration
            rbIndex = sgiListResponse.rbIndex
            rbRegionCode = sgiListResponse.rbRegionCode
            s10 = sgiListResponse.s10
            s54 = sgiListResponse.s54
            state = sgiListResponse.state
            stationName = sgiListResponse.stationName
            stce = sgiListResponse.stce.map { it.toMutableList() }
            stci = sgiListResponse.stci.map { it.toMutableList() }
            sunspotNum = sgiListResponse.sunspotNum
            sunspotNumHigh = sgiListResponse.sunspotNumHigh
            sunspotNumLow = sgiListResponse.sunspotNumLow
            transactionId = sgiListResponse.transactionId
            type = sgiListResponse.type
            y10 = sgiListResponse.y10
            y54 = sgiListResponse.y54
            additionalProperties = sgiListResponse.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [SgiListResponse].
         *
    ...
    fun validate(): SgiListResponse = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is SgiListResponse && classificationMarking == other.classificationMarking && dataMode == other.dataMode && effectiveDate == other.effectiveDate && sgiDate == other.sgiDate && source == other.source && id == other.id && analyzerAttenuation == other.analyzerAttenuation && ap == other.ap && apDuration == other.apDuration && coeffDegree == other.coeffDegree && coeffOrder == other.coeffOrder && createdAt == other.createdAt && createdBy == other.createdBy && ctce == other.ctce && ctci == other.ctci && dst == other.dst && dtc == other.dtc && e10 == other.e10 && e54 == other.e54 && f10 == other.f10 && f10High == other.f10High && f10Low == other.f10Low && f54 == other.f54 && f81 == other.f81 && gamma == other.gamma && idSensor == other.idSensor && kIndex == other.kIndex && kp == other.kp && kpDuration == other.kpDuration && m10 == other.m10 && m54 == other.m54 && mode == other.mode && normFactor == other.normFactor && origin == other.origin && origNetwork == other.origNetwork && origSensorId == other.origSensorId && precedence == other.precedence && rawFileUri == other.rawFileUri && rbDuration == other.rbDuration && rbIndex == other.rbIndex && rbRegionCode == other.rbRegionCode && s10 == other.s10 && s54 == other.s54 && state == other.state && stationName == other.stationName && stce == other.stce && stci == other.stci && sunspotNum == other.sunspotNum && sunspotNumHigh == other.sunspotNumHigh && sunspotNumLow == other.sunspotNumLow && transactionId == other.transactionId && type == other.type && y10 == other.y10 && y54 == other.y54 && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(classificationMarking, dataMode, effectiveDate, sgiDate, source, id, analyzerAttenuation, ap, apDuration, coeffDegree, coeffOrder, createdAt, createdBy, ctce, ctci, dst, dtc, e10, e54, f10, f10High, f10Low, f54, f81, gamma, idSensor, kIndex, kp, kpDuration, m10, m54, mode, normFactor, origin, origNetwork, origSensorId, precedence, rawFileUri, rbDuration, rbIndex, rbRegionCode, s10, s54, state, stationName, stce, stci, sunspotNum, sunspotNumHigh, sunspotNumLow, transactionId, type, y10, y54, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "SgiListResponse{classificationMarking=$classificationMarking, dataMode=$dataMode, effectiveDate=$effectiveDate, sgiDate=$sgiDate, source=$source, id=$id, analyzerAttenuation=$analyzerAttenuation, ap=$ap, apDuration=$apDuration, coeffDegree=$coeffDegree, coeffOrder=$coeffOrder, createdAt=$createdAt, createdBy=$createdBy, ctce=$ctce, ctci=$ctci, dst=$dst, dtc=$dtc, e10=$e10, e54=$e54, f10=$f10, f10High=$f10High, f10Low=$f10Low, f54=$f54, f81=$f81, gamma=$gamma, idSensor=$idSensor, kIndex=$kIndex, kp=$kp, kpDuration=$kpDuration, m10=$m10, m54=$m54, mode=$mode, normFactor=$normFactor, origin=$origin, origNetwork=$origNetwork, origSensorId=$origSensorId, precedence=$precedence, rawFileUri=$rawFileUri, rbDuration=$rbDuration, rbIndex=$rbIndex, rbRegionCode=$rbRegionCode, s10=$s10, s54=$s54, state=$state, stationName=$stationName, stce=$stce, stci=$stci, sunspotNum=$sunspotNum, sunspotNumHigh=$sunspotNumHigh, sunspotNumLow=$sunspotNumLow, transactionId=$transactionId, type=$type, y10=$y10, y54=$y54, additionalProperties=$additionalProperties}"
}
