...
class ChannelUpdateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(channelUpdateParams: ChannelUpdateParams) =
            apply {
                pathId = channelUpdateParams.pathId
    ...
    class Body private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (idTransponder.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (bodyId.asKnown().isPresent) 1 else 0) + (if (apid.asKnown().isPresent) 1 else 0) + (if (beamName.asKnown().isPresent) 1 else 0) + (if (compression.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (encryption.asKnown().isPresent) 1 else 0) + (if (idBeam.asKnown().isPresent) 1 else 0) + (if (idRfBand.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (owner.asKnown().isPresent) 1 else 0) + (if (pkg.asKnown().isPresent) 1 else 0) + (if (res.asKnown().isPresent) 1 else 0) + (if (sid.asKnown().isPresent) 1 else 0) + (if (type.asKnown().isPresent) 1 else 0) + (if (vpid.asKnown().isPresent) 1 else 0)
        ...
        override fun toString() = "Body{classificationMarking=$classificationMarking, dataMode=$dataMode, idTransponder=$idTransponder, name=$name, source=$source, bodyId=$bodyId, apid=$apid, beamName=$beamName, compression=$compression, createdAt=$createdAt, createdBy=$createdBy, encryption=$encryption, idBeam=$idBeam, idRfBand=$idRfBand, origin=$origin, origNetwork=$origNetwork, owner=$owner, pkg=$pkg, res=$res, sid=$sid, type=$type, vpid=$vpid, additionalProperties=$additionalProperties}"
    }
    ...
    override fun toString() = "ChannelUpdateParams{pathId=$pathId, body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
