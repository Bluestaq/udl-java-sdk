...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [SgiTupleResponse].
         *
    ...
    /** A builder for [SgiTupleResponse]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(sgiTupleResponse: SgiTupleResponse) = apply {
            classificationMarking = sgiTupleResponse.classificationMarking
            dataMode = sgiTupleResponse.dataMode
            effectiveDate = sgiTupleResponse.effectiveDate
            sgiDate = sgiTupleResponse.sgiDate
            source = sgiTupleResponse.source
            id = sgiTupleResponse.id
            analyzerAttenuation = sgiTupleResponse.analyzerAttenuation
            ap = sgiTupleResponse.ap
            apDuration = sgiTupleResponse.apDuration
            coeffDegree = sgiTupleResponse.coeffDegree.map { it.toMutableList() }
            coeffOrder = sgiTupleResponse.coeffOrder.map { it.toMutableList() }
            createdAt = sgiTupleResponse.createdAt
            createdBy = sgiTupleResponse.createdBy
            ctce = sgiTupleResponse.ctce.map { it.toMutableList() }
            ctci = sgiTupleResponse.ctci.map { it.toMutableList() }
            dst = sgiTupleResponse.dst
            dtc = sgiTupleResponse.dtc
            e10 = sgiTupleResponse.e10
            e54 = sgiTupleResponse.e54
            f10 = sgiTupleResponse.f10
            f10High = sgiTupleResponse.f10High
            f10Low = sgiTupleResponse.f10Low
            f54 = sgiTupleResponse.f54
            f81 = sgiTupleResponse.f81
            frequencies = sgiTupleResponse.frequencies.map { it.toMutableList() }
            gamma = sgiTupleResponse.gamma
            idSensor = sgiTupleResponse.idSensor
            kIndex = sgiTupleResponse.kIndex
            kp = sgiTupleResponse.kp
            kpDuration = sgiTupleResponse.kpDuration
            m10 = sgiTupleResponse.m10
            m54 = sgiTupleResponse.m54
            mode = sgiTupleResponse.mode
            normFactor = sgiTupleResponse.normFactor
            observedBaseline = sgiTupleResponse.observedBaseline.map { it.toMutableList() }
            origin = sgiTupleResponse.origin
            origNetwork = sgiTupleResponse.origNetwork
            origSensorId = sgiTupleResponse.origSensorId
            powers = sgiTupleResponse.powers.map { it.toMutableList() }
            precedence = sgiTupleResponse.precedence
            rawFileUri = sgiTupleResponse.rawFileUri
            rbDuration = sgiTupleResponse.rbDuration
            rbIndex = sgiTupleResponse.rbIndex
            rbRegionCode = sgiTupleResponse.rbRegionCode
            s10 = sgiTupleResponse.s10
            s54 = sgiTupleResponse.s54
            state = sgiTupleResponse.state
            stationName = sgiTupleResponse.stationName
            stce = sgiTupleResponse.stce.map { it.toMutableList() }
            stci = sgiTupleResponse.stci.map { it.toMutableList() }
            sunspotNum = sgiTupleResponse.sunspotNum
            sunspotNumHigh = sgiTupleResponse.sunspotNumHigh
            sunspotNumLow = sgiTupleResponse.sunspotNumLow
            tags = sgiTupleResponse.tags.map { it.toMutableList() }
            transactionId = sgiTupleResponse.transactionId
            type = sgiTupleResponse.type
            updatedAt = sgiTupleResponse.updatedAt
            updatedBy = sgiTupleResponse.updatedBy
            y10 = sgiTupleResponse.y10
            y54 = sgiTupleResponse.y54
            additionalProperties = sgiTupleResponse.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [SgiTupleResponse].
         *
    ...
    fun validate(): SgiTupleResponse = apply {
        if (validated) {
            return@apply
        }
