...
class HistoryRetrieveResponse private constructor(
    ...
    override fun toString() = "HistoryRetrieveResponse{classificationMarking=$classificationMarking, dataMode=$dataMode, recType=$recType, source=$source, startTime=$startTime, type=$type, id=$id, collectRequests=$collectRequests, createdAt=$createdAt, createdBy=$createdBy, customer=$customer, endTime=$endTime, idSensor=$idSensor, name=$name, origin=$origin, origNetwork=$origNetwork, origSensorId=$origSensorId, purpose=$purpose, reqTotal=$reqTotal, senNetwork=$senNetwork, status=$status, tags=$tags, updatedAt=$updatedAt, updatedBy=$updatedBy, additionalProperties=$additionalProperties}"
}
