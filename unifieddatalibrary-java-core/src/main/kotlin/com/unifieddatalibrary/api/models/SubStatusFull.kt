...
class SubStatusFull private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [SubStatusFull].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(subStatusFull: SubStatusFull) =
            apply {
                classificationMarking = subStatusFull.classificationMarking
                dataMode = subStatusFull.dataMode
                notes = subStatusFull.notes
                source = subStatusFull.source
                status = subStatusFull.status
                statusId = subStatusFull.statusId
                type = subStatusFull.type
                id = subStatusFull.id
                createdAt = subStatusFull.createdAt
                createdBy = subStatusFull.createdBy
                origin = subStatusFull.origin
                origNetwork = subStatusFull.origNetwork
                updatedAt = subStatusFull.updatedAt
                updatedBy = subStatusFull.updatedBy
                additionalProperties = subStatusFull.additionalProperties.toMutableMap()
            }
        ...
        /**
         * Returns an immutable instance of [SubStatusFull].
         *
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (classificationMarking.asKnown().isPresent) 1 else 0) + (dataMode.asKnown().getOrNull()?.validity() ?: 0) + (if (notes.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (statusId.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (origin.asKnown().isPresent) 1 else 0) + (if (origNetwork.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (updatedBy.asKnown().isPresent) 1 else 0)
    ...
    override fun toString() = "SubStatusFull{classificationMarking=$classificationMarking, dataMode=$dataMode, notes=$notes, source=$source, status=$status, statusId=$statusId, type=$type, id=$id, createdAt=$createdAt, createdBy=$createdBy, origin=$origin, origNetwork=$origNetwork, updatedAt=$updatedAt, updatedBy=$updatedBy, additionalProperties=$additionalProperties}"
}
