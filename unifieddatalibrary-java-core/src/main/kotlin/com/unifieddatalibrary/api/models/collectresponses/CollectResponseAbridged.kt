...
/**
 * Collect response supports the response and status of individual collect requests. Each response
 * is referenced by the UUID of the request, and contains information including the status of the
 * request, collection times and types, and reference(s) to the observations collected. There may be
 * multiple responses associated with a request, either from multiple collectors or to relay status
...
private constructor(
    ...
    /**
     * Optional identifier provided by data source to indicate the target object of this response.
     ...
     * responding to this collect or contact. This may be an internal identifier and not necessarily
     * a valid sensor ID.
     *
     * @throws UnifieddatalibraryInvalidDataException if the JSON field has an unexpected type (e.g.
     *   if the server responded with an unexpected value).
    ...
    /**
     * Array of UUIDs of the UDL data record(s) collected in response to the associated request. See
     * the associated 'srcTyps' array for the specific types of data, positionally corresponding to
     * the UUIDs in this array. The 'srcTyps' and 'srcIds' arrays must match in size. The
    ...
    /**
     * Array of UDL record type(s) (DOA, ELSET, EO, RADAR, RF, SV) collected or produced in response
     * to the associated request. See the associated 'srcIds' array for the record UUIDs,
     ...
     * systems completed and delivered constitute an equivalent successful end state. A DELIVERED
     * state is typically used for systems that exhibit a delay between collect and delivery, such
     * as with space-based systems which require ground contact to deliver observations.
     *
     ...
     *
     * PROPOSED: Indicates that the request was received and alternate collect or contact time(s)
     * (altStartTime, altEndTime) have been proposed. If an alternate is accepted by the requestor
     * the current request should be cancelled and a new request created.
     *
     * REJECTED: The request has been received and rejected by the provider. A REJECTED status may
     * be accompanied by an explanation (notes) of the reason that the request was rejected.
     *
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [CollectResponseAbridged].
         *
    ...
    /** A builder for [CollectResponseAbridged]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(collectResponseAbridged: CollectResponseAbridged) = apply {
            classificationMarking = collectResponseAbridged.classificationMarking
            dataMode = collectResponseAbridged.dataMode
            idRequest = collectResponseAbridged.idRequest
            source = collectResponseAbridged.source
            id = collectResponseAbridged.id
            actualEndTime = collectResponseAbridged.actualEndTime
            actualStartTime = collectResponseAbridged.actualStartTime
            altEndTime = collectResponseAbridged.altEndTime
            altStartTime = collectResponseAbridged.altStartTime
            createdAt = collectResponseAbridged.createdAt
            createdBy = collectResponseAbridged.createdBy
            errCode = collectResponseAbridged.errCode
            externalId = collectResponseAbridged.externalId
            idOnOrbit = collectResponseAbridged.idOnOrbit
            idPlan = collectResponseAbridged.idPlan
            idSensor = collectResponseAbridged.idSensor
            notes = collectResponseAbridged.notes
            origin = collectResponseAbridged.origin
            origNetwork = collectResponseAbridged.origNetwork
            origObjectId = collectResponseAbridged.origObjectId
            origSensorId = collectResponseAbridged.origSensorId
            satNo = collectResponseAbridged.satNo
            srcIds = collectResponseAbridged.srcIds.map { it.toMutableList() }
            srcTyps = collectResponseAbridged.srcTyps.map { it.toMutableList() }
            status = collectResponseAbridged.status
            taskId = collectResponseAbridged.taskId
            additionalProperties = collectResponseAbridged.additionalProperties.toMutableMap()
        }
         ...
         *
         * You should usually call [Builder.actualEndTime] with a well-typed [OffsetDateTime] value
         * instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
         ...
         *
         * You should usually call [Builder.altStartTime] with a well-typed [OffsetDateTime] value
         * instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
         ...
         *
         * You should usually call [Builder.errCode] with a well-typed [String] value instead. This
         * method is primarily for setting the field to an undocumented or not yet supported value.
         */
        ...
        /**
         * Returns an immutable instance of [CollectResponseAbridged].
         *
         ...
         */
        fun build(): CollectResponseAbridged =
    ...
    fun validate(): CollectResponseAbridged = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun toString() =
        "CollectResponseAbridged{classificationMarking=$classificationMarking, dataMode=$dataMode, idRequest=$idRequest, source=$source, id=$id, actualEndTime=$actualEndTime, actualStartTime=$actualStartTime, altEndTime=$altEndTime, altStartTime=$altStartTime, createdAt=$createdAt, createdBy=$createdBy, errCode=$errCode, externalId=$externalId, idOnOrbit=$idOnOrbit, idPlan=$idPlan, idSensor=$idSensor, notes=$notes, origin=$origin, origNetwork=$origNetwork, origObjectId=$origObjectId, origSensorId=$origSensorId, satNo=$satNo, srcIds=$srcIds, srcTyps=$srcTyps, status=$status, taskId=$taskId, additionalProperties=$additionalProperties}"
}
