...
class ItemTupleResponse private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [ItemTupleResponse].
         *
    ...
    /** A builder for [ItemTupleResponse]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(itemTupleResponse: ItemTupleResponse) =
            apply {
                classificationMarking = itemTupleResponse.classificationMarking
                dataMode = itemTupleResponse.dataMode
                scanCode = itemTupleResponse.scanCode
                source = itemTupleResponse.source
                type = itemTupleResponse.type
                id = itemTupleResponse.id
                accSysKeys = itemTupleResponse.accSysKeys.map { it.toMutableList() }
                accSysNotes = itemTupleResponse.accSysNotes
                accSystem = itemTupleResponse.accSystem
                accSysValues = itemTupleResponse.accSysValues.map { it.toMutableList() }
                airdrop = itemTupleResponse.airdrop
                altDataFormat = itemTupleResponse.altDataFormat
                cargoType = itemTupleResponse.cargoType
                centerlineOffset = itemTupleResponse.centerlineOffset
                cg = itemTupleResponse.cg
                commodityCode = itemTupleResponse.commodityCode
                commoditySys = itemTupleResponse.commoditySys
                container = itemTupleResponse.container
                createdAt = itemTupleResponse.createdAt
                createdBy = itemTupleResponse.createdBy
                departure = itemTupleResponse.departure
                destination = itemTupleResponse.destination
                dvCode = itemTupleResponse.dvCode
                fs = itemTupleResponse.fs
                hazCodes = itemTupleResponse.hazCodes.map { it.toMutableList() }
                height = itemTupleResponse.height
                idAirLoadPlan = itemTupleResponse.idAirLoadPlan
                itemContains = itemTupleResponse.itemContains.map { it.toMutableList() }
                keys = itemTupleResponse.keys.map { it.toMutableList() }
                lastArrDate = itemTupleResponse.lastArrDate
                length = itemTupleResponse.length
                moment = itemTupleResponse.moment
                name = itemTupleResponse.name
                netExpWt = itemTupleResponse.netExpWt
                notes = itemTupleResponse.notes
                numPalletPos = itemTupleResponse.numPalletPos
                origin = itemTupleResponse.origin
                origNetwork = itemTupleResponse.origNetwork
                productCode = itemTupleResponse.productCode
                productSys = itemTupleResponse.productSys
                receivingBranch = itemTupleResponse.receivingBranch
                receivingUnit = itemTupleResponse.receivingUnit
                scGenTool = itemTupleResponse.scGenTool
                sourceDl = itemTupleResponse.sourceDl
                tcn = itemTupleResponse.tcn
                uln = itemTupleResponse.uln
                updatedAt = itemTupleResponse.updatedAt
                updatedBy = itemTupleResponse.updatedBy
                values = itemTupleResponse.values.map { it.toMutableList() }
                volume = itemTupleResponse.volume
                weight = itemTupleResponse.weight
                weightTs = itemTupleResponse.weightTs
                width = itemTupleResponse.width
                additionalProperties = itemTupleResponse.additionalProperties.toMutableMap()
            }
        ...
        /**
         * Returns an immutable instance of [ItemTupleResponse].
         *
    ...
    fun validate(): ItemTupleResponse =
        apply {
            if (validated) {
              return@apply
            }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return /* spotless:off */ other is ItemTupleResponse && classificationMarking == other.classificationMarking && dataMode == other.dataMode && scanCode == other.scanCode && source == other.source && type == other.type && id == other.id && accSysKeys == other.accSysKeys && accSysNotes == other.accSysNotes && accSystem == other.accSystem && accSysValues == other.accSysValues && airdrop == other.airdrop && altDataFormat == other.altDataFormat && cargoType == other.cargoType && centerlineOffset == other.centerlineOffset && cg == other.cg && commodityCode == other.commodityCode && commoditySys == other.commoditySys && container == other.container && createdAt == other.createdAt && createdBy == other.createdBy && departure == other.departure && destination == other.destination && dvCode == other.dvCode && fs == other.fs && hazCodes == other.hazCodes && height == other.height && idAirLoadPlan == other.idAirLoadPlan && itemContains == other.itemContains && keys == other.keys && lastArrDate == other.lastArrDate && length == other.length && moment == other.moment && name == other.name && netExpWt == other.netExpWt && notes == other.notes && numPalletPos == other.numPalletPos && origin == other.origin && origNetwork == other.origNetwork && productCode == other.productCode && productSys == other.productSys && receivingBranch == other.receivingBranch && receivingUnit == other.receivingUnit && scGenTool == other.scGenTool && sourceDl == other.sourceDl && tcn == other.tcn && uln == other.uln && updatedAt == other.updatedAt && updatedBy == other.updatedBy && values == other.values && volume == other.volume && weight == other.weight && weightTs == other.weightTs && width == other.width && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(classificationMarking, dataMode, scanCode, source, type, id, accSysKeys, accSysNotes, accSystem, accSysValues, airdrop, altDataFormat, cargoType, centerlineOffset, cg, commodityCode, commoditySys, container, createdAt, createdBy, departure, destination, dvCode, fs, hazCodes, height, idAirLoadPlan, itemContains, keys, lastArrDate, length, moment, name, netExpWt, notes, numPalletPos, origin, origNetwork, productCode, productSys, receivingBranch, receivingUnit, scGenTool, sourceDl, tcn, uln, updatedAt, updatedBy, values, volume, weight, weightTs, width, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() = "ItemTupleResponse{classificationMarking=$classificationMarking, dataMode=$dataMode, scanCode=$scanCode, source=$source, type=$type, id=$id, accSysKeys=$accSysKeys, accSysNotes=$accSysNotes, accSystem=$accSystem, accSysValues=$accSysValues, airdrop=$airdrop, altDataFormat=$altDataFormat, cargoType=$cargoType, centerlineOffset=$centerlineOffset, cg=$cg, commodityCode=$commodityCode, commoditySys=$commoditySys, container=$container, createdAt=$createdAt, createdBy=$createdBy, departure=$departure, destination=$destination, dvCode=$dvCode, fs=$fs, hazCodes=$hazCodes, height=$height, idAirLoadPlan=$idAirLoadPlan, itemContains=$itemContains, keys=$keys, lastArrDate=$lastArrDate, length=$length, moment=$moment, name=$name, netExpWt=$netExpWt, notes=$notes, numPalletPos=$numPalletPos, origin=$origin, origNetwork=$origNetwork, productCode=$productCode, productSys=$productSys, receivingBranch=$receivingBranch, receivingUnit=$receivingUnit, scGenTool=$scGenTool, sourceDl=$sourceDl, tcn=$tcn, uln=$uln, updatedAt=$updatedAt, updatedBy=$updatedBy, values=$values, volume=$volume, weight=$weight, weightTs=$weightTs, width=$width, additionalProperties=$additionalProperties}"
}
