...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(historyListResponse: HistoryListResponse) = apply {
            ...
            endDate = historyListResponse.endDate
            idLaserEmitters = historyListResponse.idLaserEmitters.map { it.toMutableList() }
            numTargets = historyListResponse.numTargets
            requestId = historyListResponse.requestId
            requestTs = historyListResponse.requestTs
            source = historyListResponse.source
            startDate = historyListResponse.startDate
            id = historyListResponse.id
            centerlineAzimuth = historyListResponse.centerlineAzimuth
            centerlineElevation = historyListResponse.centerlineElevation
            createdAt = historyListResponse.createdAt
            createdBy = historyListResponse.createdBy
            defaultCha = historyListResponse.defaultCha
            enableDss = historyListResponse.enableDss
            fixedPoints = historyListResponse.fixedPoints.map { it.toMutableList() }
            geopotentialModel = historyListResponse.geopotentialModel
            idOnOrbit = historyListResponse.idOnOrbit
            laserDeconflictTargets =
                historyListResponse.laserDeconflictTargets.map { it.toMutableList() }
            laserSystemName = historyListResponse.laserSystemName
            lengthCenterline = historyListResponse.lengthCenterline
            lengthLeftRight = historyListResponse.lengthLeftRight
            lengthUpDown = historyListResponse.lengthUpDown
            maximumHeight = historyListResponse.maximumHeight
            minimumHeight = historyListResponse.minimumHeight
            missionName = historyListResponse.missionName
            ...
            platformLocationName = historyListResponse.platformLocationName
            platformLocationType = historyListResponse.platformLocationType
            programId = historyListResponse.programId
            propagator = historyListResponse.propagator
            protectList = historyListResponse.protectList.map { it.toMutableList() }
            satNo = historyListResponse.satNo
            sourceDl = historyListResponse.sourceDl
            sourceEnabled = historyListResponse.sourceEnabled
            status = historyListResponse.status
            tags = historyListResponse.tags.map { it.toMutableList() }
            targetEnabled = historyListResponse.targetEnabled
            targetType = historyListResponse.targetType
            transactionId = historyListResponse.transactionId
            treatEarthAs = historyListResponse.treatEarthAs
            useFieldOfRegard = historyListResponse.useFieldOfRegard
            victimEnabled = historyListResponse.victimEnabled
