...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(trackListResponse: TrackListResponse) = apply {
            classificationMarking = trackListResponse.classificationMarking
            dataMode = trackListResponse.dataMode
            source = trackListResponse.source
            ts = trackListResponse.ts
            id = trackListResponse.id
            alt = trackListResponse.alt
            asset = trackListResponse.asset
            assetNat = trackListResponse.assetNat
            attitude = trackListResponse.attitude.map { it.toMutableList() }
            ...
            covReferenceFrame = trackListResponse.covReferenceFrame
            createdAt = trackListResponse.createdAt
            createdBy = trackListResponse.createdBy
            ecefAcc = trackListResponse.ecefAcc.map { it.toMutableList() }
            ...
            envConf = trackListResponse.envConf
            errEllp = trackListResponse.errEllp.map { it.toMutableList() }
            grndSpd = trackListResponse.grndSpd
            hdng = trackListResponse.hdng
            identAmp = trackListResponse.identAmp
            identCred = trackListResponse.identCred
            identRel = trackListResponse.identRel
            jSeries = trackListResponse.jSeries
            lat = trackListResponse.lat
            lcAcc = trackListResponse.lcAcc.map { it.toMutableList() }
            lco = trackListResponse.lco.map { it.toMutableList() }
            lcPos = trackListResponse.lcPos.map { it.toMutableList() }
            lcs = trackListResponse.lcs.map { it.toMutableList() }
            lcVel = trackListResponse.lcVel.map { it.toMutableList() }
            lon = trackListResponse.lon
            m1 = trackListResponse.m1
            m1v = trackListResponse.m1v
            m2 = trackListResponse.m2
            m2v = trackListResponse.m2v
            m3a = trackListResponse.m3a
            m3av = trackListResponse.m3av
            modType = trackListResponse.modType
            msgTs = trackListResponse.msgTs
            ...
            objPlat = trackListResponse.objPlat
            objSpec = trackListResponse.objSpec
            objType = trackListResponse.objType
            origin = trackListResponse.origin
            origNetwork = trackListResponse.origNetwork
            sen = trackListResponse.sen
            senQual = trackListResponse.senQual
            sourceDl = trackListResponse.sourceDl
            spd = trackListResponse.spd
            srcIds = trackListResponse.srcIds.map { it.toMutableList() }
            srcTyps = trackListResponse.srcTyps.map { it.toMutableList() }
            strength = trackListResponse.strength
            tags = trackListResponse.tags.map { it.toMutableList() }
            trkConf = trackListResponse.trkConf
            trkId = trackListResponse.trkId
            trkItmId = trackListResponse.trkItmId
            trkNum = trackListResponse.trkNum
