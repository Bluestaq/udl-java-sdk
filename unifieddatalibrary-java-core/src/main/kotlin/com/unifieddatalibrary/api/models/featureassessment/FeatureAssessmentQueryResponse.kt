 ...
 */
class FeatureAssessmentQueryResponse
private constructor(
    ...
    /** A builder for [FeatureAssessmentQueryResponse]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(featureAssessmentQueryResponse: FeatureAssessmentQueryResponse) = apply {
            classificationMarking = featureAssessmentQueryResponse.classificationMarking
            dataMode = featureAssessmentQueryResponse.dataMode
            featureTs = featureAssessmentQueryResponse.featureTs
            featureUoM = featureAssessmentQueryResponse.featureUoM
            idAnalyticImagery = featureAssessmentQueryResponse.idAnalyticImagery
            source = featureAssessmentQueryResponse.source
            id = featureAssessmentQueryResponse.id
            agjson = featureAssessmentQueryResponse.agjson
            andims = featureAssessmentQueryResponse.andims
            annLims = featureAssessmentQueryResponse.annLims.map { it.toMutableList() }
            annText = featureAssessmentQueryResponse.annText.map { it.toMutableList() }
            asrid = featureAssessmentQueryResponse.asrid
            assessment = featureAssessmentQueryResponse.assessment
            atext = featureAssessmentQueryResponse.atext
            atype = featureAssessmentQueryResponse.atype
            confidence = featureAssessmentQueryResponse.confidence
            createdAt = featureAssessmentQueryResponse.createdAt
            createdBy = featureAssessmentQueryResponse.createdBy
            externalId = featureAssessmentQueryResponse.externalId
            featureArray = featureAssessmentQueryResponse.featureArray.map { it.toMutableList() }
            featureBool = featureAssessmentQueryResponse.featureBool
            featureString = featureAssessmentQueryResponse.featureString
            featureStringArray =
                featureAssessmentQueryResponse.featureStringArray.map { it.toMutableList() }
            featureValue = featureAssessmentQueryResponse.featureValue
            heading = featureAssessmentQueryResponse.heading
            height = featureAssessmentQueryResponse.height
            length = featureAssessmentQueryResponse.length
            name = featureAssessmentQueryResponse.name
            origin = featureAssessmentQueryResponse.origin
            origNetwork = featureAssessmentQueryResponse.origNetwork
            sourceDl = featureAssessmentQueryResponse.sourceDl
            speed = featureAssessmentQueryResponse.speed
            srcIds = featureAssessmentQueryResponse.srcIds.map { it.toMutableList() }
            srcTs = featureAssessmentQueryResponse.srcTs.map { it.toMutableList() }
            srcTyps = featureAssessmentQueryResponse.srcTyps.map { it.toMutableList() }
            transactionId = featureAssessmentQueryResponse.transactionId
            type = featureAssessmentQueryResponse.type
            width = featureAssessmentQueryResponse.width
            additionalProperties =
                featureAssessmentQueryResponse.additionalProperties.toMutableMap()
        }
        ...
        /**
         * Returns an immutable instance of [FeatureAssessmentQueryResponse].
         *
         ...
         */
        fun build(): FeatureAssessmentQueryResponse =
            FeatureAssessmentQueryResponse(
    ...
    fun validate(): FeatureAssessmentQueryResponse = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is FeatureAssessmentQueryResponse && classificationMarking == other.classificationMarking && dataMode == other.dataMode && featureTs == other.featureTs && featureUoM == other.featureUoM && idAnalyticImagery == other.idAnalyticImagery && source == other.source && id == other.id && agjson == other.agjson && andims == other.andims && annLims == other.annLims && annText == other.annText && asrid == other.asrid && assessment == other.assessment && atext == other.atext && atype == other.atype && confidence == other.confidence && createdAt == other.createdAt && createdBy == other.createdBy && externalId == other.externalId && featureArray == other.featureArray && featureBool == other.featureBool && featureString == other.featureString && featureStringArray == other.featureStringArray && featureValue == other.featureValue && heading == other.heading && height == other.height && length == other.length && name == other.name && origin == other.origin && origNetwork == other.origNetwork && sourceDl == other.sourceDl && speed == other.speed && srcIds == other.srcIds && srcTs == other.srcTs && srcTyps == other.srcTyps && transactionId == other.transactionId && type == other.type && width == other.width && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(classificationMarking, dataMode, featureTs, featureUoM, idAnalyticImagery, source, id, agjson, andims, annLims, annText, asrid, assessment, atext, atype, confidence, createdAt, createdBy, externalId, featureArray, featureBool, featureString, featureStringArray, featureValue, heading, height, length, name, origin, origNetwork, sourceDl, speed, srcIds, srcTs, srcTyps, transactionId, type, width, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "FeatureAssessmentQueryResponse{classificationMarking=$classificationMarking, dataMode=$dataMode, featureTs=$featureTs, featureUoM=$featureUoM, idAnalyticImagery=$idAnalyticImagery, source=$source, id=$id, agjson=$agjson, andims=$andims, annLims=$annLims, annText=$annText, asrid=$asrid, assessment=$assessment, atext=$atext, atype=$atype, confidence=$confidence, createdAt=$createdAt, createdBy=$createdBy, externalId=$externalId, featureArray=$featureArray, featureBool=$featureBool, featureString=$featureString, featureStringArray=$featureStringArray, featureValue=$featureValue, heading=$heading, height=$height, length=$length, name=$name, origin=$origin, origNetwork=$origNetwork, sourceDl=$sourceDl, speed=$speed, srcIds=$srcIds, srcTs=$srcTs, srcTyps=$srcTyps, transactionId=$transactionId, type=$type, width=$width, additionalProperties=$additionalProperties}"
}
