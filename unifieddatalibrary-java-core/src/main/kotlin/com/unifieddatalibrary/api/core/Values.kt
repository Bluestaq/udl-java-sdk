...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { UnifieddatalibraryInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw UnifieddatalibraryInvalidDataException("`$name` is not set")
            is JsonNull -> throw UnifieddatalibraryInvalidDataException("`$name` is null")
            else ->
                throw UnifieddatalibraryInvalidDataException("`$name` is invalid, received $this")
        }
